...
public class DepartmentTest {
    ...
    public static List<Department> getSub(int pid, List<Department> allDepartment) {
        ...
        List<Department> pidde = allDepartment.stream().filter(da -> da.getPid() == pid).collect(Collectors.toList());

        if (CollectionUtils.isEmpty(pidde)) {
            return Lists.newArrayList();
        }

        List<Integer> collect = pidde.stream().map(Department::getId).collect(Collectors.toList());

        List<Department> departments = Lists.newArrayList(pidde);
        collect.forEach(x -> {
            departments.addAll(getSub(x, allDepartment));

        });
        return  departments;

//        List<Department> childDepartmentList = allDepartment.stream()
//                .filter(department -> department.getPid() == pid)
//                .collect(Collectors.toList());
//        if (CollectionUtils.isEmpty(childDepartmentList)) {
//            return Lists.newArrayList();
//        }
//
//        List<Integer> childDepartmentIds = childDepartmentList.stream()
//                .map(Department::getId)
//                .collect(Collectors.toList());
//        List<Department> departments = Lists.newArrayList(childDepartmentList);
//        childDepartmentIds.forEach(child -> {
//            departments.addAll(getSub(child, allDepartment));
//        });
//        return departments;
    }
}
